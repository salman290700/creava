...
func (h *Handler) LoginWithGoogle(c *gin.Context) {
	...
	// Get data from google token api
	...
	// Service
	token, refresh_token, setStatusCode, err := h.creatorService.LoginWithGoogle(ctx, &res_user_google)
	if err != nil {
		c.JSON(int(setStatusCode), gin.H{
			"message": err,
		})
		return
	}
	// Response
	c.JSON(int(setStatusCode), dto.LoginResponse{
		Token:        token,
